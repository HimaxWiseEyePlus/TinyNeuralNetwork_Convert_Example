...
def main_worker(args):
    with model_tracer():
        ...
        train(model, context, train_one_epoch, validate, work_dir='out_qat_pruning')
        ...
        quantizer = QATQuantizer(model, dummy_input, work_dir='out_qat_pruning')
    ...
    train(qat_model, context, train_one_epoch, validate, qat=True, work_dir='out_qat_pruning')
    ...
    with torch.no_grad():
        ...
        converter = TFLiteConverter(qat_model, dummy_input, tflite_path='out_qat_pruning/qat_pruning_model.tflite',quantize_input_output_type="int8",fuse_quant_dequant=True,quantize_target_type="int8")
        converter.convert()
